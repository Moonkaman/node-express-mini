...
server.delete('/api/users/:id', (req, res) => {
  const userID = req.params.id;
  db.remove(userID)
    .then(del => del === 0 ? res.status(404).json({error: 'The user with the specified ID does not exist.'}) : db.find().then(users => res.status(200).json(users)).catch(_ => res.status(500).json({error: 'error retrieving users.'})))
